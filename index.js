...
/* A collection of quotes from SNL's Deep Thoughts with Jack Handey */
/* These are here just for fun, and because I couldn't find a quote API to serve them */
var quotes = [
...
/* An array of background colors to randomly set the background for each quote */
var colors=["#337ab7","#5cb85c","#5bc0de","#f0ad4e","#d9534f","#784FC4", "#b43c98","#000000"];


/* Event handler for the "Be Amused button" */
/* Sets the text to a random quote from the Deep Thoughts array
   Sets the author to Jack Handey
...
/* This function parses the forismatic quote and updates the quote and author section of the page */
...
/* Sets the tweet value to the author + the quote + the RandomQuote URL.
   Twitter modifies the URL, so 117 characters are left for the quote
   The quote is truncated if total length exceeds twitter's max 140 characters */
function tweetIt() {
    var auth = document.getElementById('author').innerText.concat(": ");
    var phrase = auth.concat(document.getElementById('randomQuote').innerText.substr(0,(117-auth.length)).trim());
  ...
  var tweetUrl = 'https://twitter.com/share?text=' +
    ...
    '&url=' +
    'http://genestd.github.io/RQM/';

  window.open(tweetUrl);
}

/* Get a quote on the initial load */
 $(document).ready(function() {
 $.getJSON("http://api.forismatic.com/api/1.0/?method=getQuote&lang=en&format=jsonp&jsonp=?")
    .done(update)
    .fail(handleErr);
  });
Marcel Proust: Let us be grateful to people who make us happy; they are the charming gardeners who make our souls... http://codepen.io/dgenest/full/xOEorg/
